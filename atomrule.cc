...
void
Atom::addToRuleList(NestedRule* rule){
  assert(rule);
  headof++;
...
// ..b..:-..a.. 
void 
Atom::checkDoubleCycle(){// go thru list of all positive atoms
...
void 
Atom::propagateInBodies(bool value){// go thru list of all rules
                        // and sets Counters
...
void 
Atom::propagateComputeTrueInNegBodies(){// go thru list of all rules
                        // and sets Counters
   for(list<Rule*>::iterator itrR=negBodyRules.begin();
	  itrR!=negBodyRules.end();
	  itrR++){
...
CompareValues
FunctionalityRule::equal(Atom** sup,  Atom** supend,Atom** sub,  Atom** subend){
  if(supend-sup>subend-sub)
	return GTH;
...
CompareValues
Rule::bodySubsumed(Atom** nbody1,Atom** pbody1,Atom**nnbody1,Atom**end1,
...
CompareValues
FunctionalityRule::bodyEq(Atom** nbody1,Atom** pbody1,Atom**nnbody1,Atom**end1,
				   Atom** nbody2,Atom** pbody2,Atom**nnbody2,Atom**end2){
...
bool  
Rule::atomInList(Atom* a, Atom** la,Atom** laend){
  for(la;la<laend;la++){
	if((*la)==a)
	  return true;
  }
...
BasicRule::cmpRule(Rule* rcmp ){
  ...
  if(rcmp->type==BASICRULE){//as this rule
	...
	if(cmp!=EQ)
	  return cmp;
	return bodySubsumed(nbody,pbody,end,end,
						r2cmp->nbody,r2cmp->pbody,r2cmp->end,r2cmp->end);
  }
  if(rcmp->type==CHOICERULE){//as this rule
	ChoiceRule* r2cmp=(ChoiceRule*)rcmp;
	if(r2cmp->head==head){
	  CompareValues cmp= bodySubsumed(nbody,pbody,end,end,
						r2cmp->nbody,r2cmp->pbody,r2cmp->end,r2cmp->end);
  ...
  if(rcmp->type==DISJUNCTIONRULE){//basic_rule is smaller than any other rule
...
CompareValues 
ChoiceRule::cmpRule(Rule* rcmp ){
  if(rcmp->type==CONSTRAINTRULE||rcmp->type==WEIGHTRULE||rcmp->type==DISJUNCTIONRULE)
	return LTH;
  if(rcmp->type==BASICRULE){//as this rule
	BasicRule* r2cmp=(BasicRule*)rcmp;
...
CompareValues 
DisjunctionRule::cmpRule(Rule* rcmp ){
  if(rcmp->type==CONSTRAINTRULE||rcmp->type==WEIGHTRULE)
	return LTH;
  ...
  if(hsub==LTH||hsub==GTH)
	return hsub;
  CompareValues bsub = bodySubsumed(nbody,pbody,end,end,
					  r2cmp->nbody,r2cmp->pbody,r2cmp->end,r2cmp->end);
  if(hsub==EQ)
	return bsub;
  if(bsub==EQ)
...
CompareValues 
ConstraintRule::cmpRule(Rule* rcmp ){
  ...
  return bodyEq(nbody,pbody,end,end,
				r2cmp->nbody,r2cmp->pbody,r2cmp->end,r2cmp->end);  
}

CompareValues
WeightRule::bodyEqWr(WeightRule* wr){
...
CompareValues 
NestedRule::cmpBody(NestedRule* r2cmp ){
  NestedRule* rcmp=(NestedRule*) r2cmp;
  return bodyEq(nbody,pbody,nnbody,end,
				rcmp->nbody,rcmp->pbody,rcmp->nnbody,rcmp->end);  
}
...
Atom**
ChoiceRule::posendbody(){
  //returns positive part of the body
  return pend;
}
Atom**
DisjunctionRule::posbody(){
  //returns positive part of the body
  return pbody;
}
Atom**
DisjunctionRule::posendbody(){
  //returns positive part of the body
  return pend;
}
Atom**
ConstraintRule::posbody(){
  //returns positive part of the body
  return 0;
}
Atom**
ConstraintRule::posendbody(){
  //returns positive part of the body
  return 0;
}
Atom**
...
Atom**
WeightRule::posendbody(){
  //returns positive part of the body
  return 0;
}

void 
BasicRule::propUpper(Atom* a){
  upper--;
...
void 
DisjunctionRule::propUpper(Atom* a){
  upper--;
  if(upper==0&&satUUn!=UNSAT){
...
void 
ConstraintRule::propUpper(Atom* a){
  upper--;
  if(upper==0&&satUUn!=UNSAT){
	head->p->q.push(head);
  }
...
void 
BasicRule::HeadInOneRule(Atom* at){
  Atom** a;
...
void 
ChoiceRule::HeadInOneRule(Atom* at){
  Atom** a;
  for(a=nbody; a<nend; a++){
	(*a)->setBFalse();
  }
...
void 
ConstraintRule::HeadInOneRule(Atom* at){
  Atom** a;
...
void 
WeightRule::HeadInOneRule(Atom* at){
  Atom** a;
  if(atleastCount<=maxweightCount){
	for(Atom** a=body; a<end; a++){
	  if(!(*a)->Bneg&&!(*a)->computeFalse&&!(*a)->Bpos&&!(*a)->computeTrue){
...
void 
DisjunctionRule::HeadInOneRule(Atom* at){
  Atom** a;
...
void 
Atom::propagateConsInBodies(){// go thru list of pBodyRules
                        // and sets pbodyCount
...
//
// Sets counters and active by propogating pos/neg atom in pos/neg part of the body
//
void
BasicRule::BodyAtom(bool posBody, bool posAtom, Atom* at){
  if(satUUn!=UNKNOWN)
	return;
  if(posBody&&posAtom){
	posBodyCount--;
...
//
void
DisjunctionRule::HeadAtom(bool value, Atom* at){
  if (!value){
	negHeadCount++;
	...
	if(hend-head-negHeadCount==1&& posBodyCount+negBodyCount==0){
	  ...
	  //and body of a rule is satisfied, hence the atom which is left is setToTrue
	  for(Atom** a=head; a<hend; a++){ 
		if(!(*a)->Bneg){
		  if(allAtomsBpos())
			(*a)->setBTrue();
...
//
void
DisjunctionRule::BodyAtom(bool posBody, bool posAtom, Atom* at){
  if(satUUn!=UNKNOWN)
	return;
  if(posBody&&posAtom){
	posBodyCount--;
...
//
void
ChoiceRule::BodyAtom(bool posBody, bool posAtom, Atom* at){
...
//
void
ConstraintRule::BodyAtom(bool posBody, bool posAtom, Atom* at){
  if(satUUn!=UNKNOWN)
	return;
  if(posBody&&posAtom){
	posBodyCount--;
...
bool
ChoiceRule::allAtomsBpos(){
  return false;
}

Weight
WeightRule::findAtWeightBody(Atom* at,  bool posBody){
  Atom **a;
  if(posBody){
	for(a=body; a<end; a++){
	  if(aux[a-body].positive)
...
//
void
WeightRule::BodyAtom(bool posBody, bool posAtom, Atom* at){
  if(satUUn!=UNKNOWN)
	return;
  int ind=0;
...
void 
ChoiceRule::propagateHeadFalse (){
  //nothing is done as with choice rule this does not add us info
}
void 
ConstraintRule::propagateHeadFalse (){
  if(satUUn==UNSAT)
	return;
...
void 
WeightRule::propagateHeadFalse (){
  //need to put more thinking in
  //for now nothing
}
void 
ChoiceRule::HeadAtom (bool v, Atom* at){
  //nothing is done as with choice rule this does not add us info
}
void
BasicRule::HeadAtom (bool v, Atom* at){
  //nothing is done as with choice rule this does not add us info
}
void 
ConstraintRule::HeadAtom (bool v, Atom* at){
  //need to put more thinking in
  //for now nothing

}
void 
WeightRule::HeadAtom (bool v, Atom* at){
  //need to put more thinking in
  //for now nothing
}

void 
DisjunctionRule::propagateHeadFalse (){
...
BasicRule::satUnsatUnknown ()
{
  ...
  return satUUn; //now it will be known at time of atom propogating if rule is unsat
}
Result
ChoiceRule::satUnsatUnknown ()
{  
  if(satUUn==SAT||satUUn==UNSAT)
	return satUUn;
  if(posBodyCount+negBodyCount==0){
...
Result
DisjunctionRule::satUnsatUnknown ()
{
  if(satUUn==SAT||satUUn==UNSAT)
	return satUUn;
...
Result
ConstraintRule::satUnsatUnknown ()
{
  if(satUUn==SAT||satUUn==UNSAT)
	return satUUn;
...
void
BasicRule::init (Init *init, int pos)
{
  head = *init->head;
...
void BasicRule::initHeadRuleLists4Sort(){
  head->headRules.push_back(this);
}
void ChoiceRule::initHeadRuleLists4Sort(){
  head->headRules.push_back(this);
}
void WeightRule::initHeadRuleLists4Sort(){
  head->headRules.push_back(this);
}
...
void
ConstraintRule::init (Init *init, int pos)
{
  head = *init->head;
...
void 
ConstraintRule::initRuleLists4WF(){
  head->headof++;
  head->headRules.push_back(this);
  for(Atom** a=nbody;a<nend;a++)
	(*a)->negBodyRules.push_back(this);
...
void
ChoiceRule::init (Init *init, int pos)
{
  head = init->head[pos];
  long n = init->psize+init->nsize;
  upper=init->psize;
...
void
DisjunctionRule::init (Init *init, int pos)
{
...
void 
DisjunctionRule::initRuleLists4WF(){
  for(Atom** a=head;a<hend;a++){
	(*a)->headof++;
	(*a)->headRules.push_back(this);
  }
...
void
WeightRule::init (Init *init, int pos)
{
  head = *init->head;
...
void 
Completion::allocateNbody(long size){
  nbody=new  Atom *[size];
  end =nbody+size;
}

void 
Completion::initCompletionNbodyFromApi(Api* api){
...
void 
Completion::initCompletionNbodyFromCompApi(Api* api){
...
void 
NestedRule::initRuleFromApi(Api* api, RuleType ruleType){
  type = ruleType;

  int size = api->sizeNbody() + api->sizePbody() + api->sizeHead() + api->sizeNNbody();
...
void 
Clause::initClauseFromApi(Api* api){
...
NestedRule::sizeHead(){
  ...
  long
NestedRule::sizePbody(){
  return numPbody;
}
  long
NestedRule::sizeNbody(){
  return numNbody;
}
  long
NestedRule::sizeBody(){
  return numNbody+numPbody+numNNbody;
}
  long
NestedRule::sizeNNbody(){
  return numNNbody;
}
...
NestedRule::getNbody(){
  ...
  Atom ** 
NestedRule::getNNbody(){
  return nnbody;
}


  void 
NestedRule::allocateRule(long headSize, long nbodySize,
							 long pbodySize,long nnbodySize){
  ...
  void 
NestedRule::addHead(long pos,Atom *atom){ 
  ...
  void 
NestedRule::addNbody(long pos, Atom *atom){  
  assert(pos>=0&&pos<numNbody);
  nbody[pos]=atom;
}

  void 
NestedRule::addPbody(long pos, Atom *atom){  
  ...
  void 
NestedRule::addNNbody(long pos, Atom *atom){  
  assert(pos>=0&&pos<numNNbody);
  nnbody[pos]=atom;
}
  void 
NestedRule::addBody(long pos, Atom *atom){  
  assert(pos>=0&&pos<sizeBody());
  nbody[pos]=atom;
}

  void 
NestedRule::allocateHead(long size){
  head=new  Atom *[size];
  end =head+size;
}
...
Clause::allocate(long size){
  ...
  void 
Clause::allocateClause(long nbodySize, long pbodySize){
  ...
  void 
Clause::addNbody(long pos, Atom *atom){  
  assert(pos>=0&&pos<numNbody);
  nbody[pos]=atom;
}

  void 
Clause::addPbody(long pos, Atom *atom){  
  ...
  void 
Clause::addBody(long pos, Atom *atom){  
